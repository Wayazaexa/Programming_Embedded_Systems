...
GPIOA->MODER &=~0x00F0; /* Set bits [4:7] in the GPIOA_MODER register to 0.
                           This sets MODER2 and MODER3 (pin 2 and pin 3 if I interpret 
                           it correctly) to be inputs. */
GPIOA->MODER |= 0x00A0; /* Set bits 5 and 7 in the GPIOA_MODER register to 1.
                           ...
                           GPIOA_MODER register to 1010, which sets pins 2 and 3 to
...
USART2->BRR  =  0x0683; /* Sets the USART2 Baud Rate Register to 0x0683, which means
                           that the mantissa of the USART Divider is set to 68,
...
int USART2_write(int ch){ /* Start inline definition of USART2_write(int). */
	...
	while(!(USART2->SR & 0x0080)){} /* Wait while bit 7 in USART2_SR (TXE) is 1, as
                                          this indicate the USART is busy transmitting. */
...
int USART2_read(void){ /* Start inline definition of USART2_read(). */
  while(!(USART2->SR & 0x0020)){} /* Wait while bit 5 in USART2_SR (RXNE) is 1, as this
                                     indicate <<<<<<<< SOMETHING >>>>>>>>.
                                    (most likely that the USART is busy receiving data) */
